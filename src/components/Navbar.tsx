  ...
  return (
    <header className="nav flex items-center justify-between sm:px-[20px] sm:py-4 md:px-20 md:pt-10 lg:px-[60px] lg:py-4 2xl:px-[288px] 2xl:py-10 relative">
      ...
      <nav className="desktopNav hidden! lg:flex! sm:max-xl:gap-6 xl:gap-10! items-center">
        <NavLink to="/mainboard" className="text-[16px] font-semibold xl:text-[24px]" end>MAIN BOARD</NavLink>
        <NavLink to="/infocus" className="text-[16px] font-semibold xl:text-[24px]">INFOCUS</NavLink>
        ...
        >
          <NavLink to="/models" className="nav-dropdown-trigger text-[16px] xl:text-[24px] inline-flex items-center gap-2">
            MODELS <span className={`dropdown-arrow ${isModelsOpen ? "open" : ""}`}>▼</span>
          </NavLink>

          {isModelsOpen && (
            <div className="nav-submenu absolute mt-2 bg-white shadow p-3">
              <NavLink to="/models/kids" className="text-[16px] font-semibold xl:text-[24px]">Kids</NavLink>
              <NavLink to="/models/teens" className="text-[16px] font-semibold xl:text-[24px]">Teens</NavLink>
              <NavLink to="/models/thai" className="text-[16px] font-semibold xl:text-[24px]">Thai</NavLink>
            </div>
          )}
        </div>

        <NavLink to="/grwmrr" className="text-[16px] font-semibold xl:text-[24px]">GRWMRR</NavLink>
        <NavLink to="/contact" className="text-[16px] font-semibold xl:text-[24px]">CONTACT US</NavLink>
      </nav>

      {/* MOBILE MENU */}
      {isMobileOpen && (
        <div className="lg:hidden flex flex-col gap-8 fixed top-0 right-0 h-full w-[70%] bg-[#d5d5d5] sm:p-10 sm:pt-20 md:pt-20 md:p-16 shadow-lg z-9999">
          {/* Close button */}
          <button className="absolute top-6 right-6 text-4xl" onClick={handleClose}>×</button>
          <NavLink to="/mainboard"  className="sm:text-[16px] md:text-[20px] font-semibold xl:text-[24px]" end onClick={handleClose}>MAIN BOARD</NavLink>
          <NavLink to="/infocus"  className="sm:text-[16px] md:text-[20px] font-semibold xl:text-[24px]" onClick={handleClose}>INFOCUS</NavLink>

          {/* MODELS dropdown */}
          <div>
            <div className="flex items-center gap-4 cursor-pointer">
              <NavLink to="/models"  className="sm:text-[16px] md:text-[20px] font-semibold xl:text-[24px]" onClick={handleClose}>MODELS</NavLink>
            ...
            {isModelsOpen && (
              <div className="flex flex-col ml-4 gap-2 mt-2">
                <NavLink to="/models/kids" className="sm:text-[16px] md:text-[20px] font-semibold xl:text-[24px]" onClick={handleClose}>Kids</NavLink>
                <NavLink to="/models/teens" className="sm:text-[16px] md:text-[20px] font-semibold xl:text-[24px]" onClick={handleClose}>Teens</NavLink>
                <NavLink to="/models/thai" className="sm:text-[16px] md:text-[20px] font-semibold xl:text-[24px]" onClick={handleClose}>Thai</NavLink>
              </div>
            )}
          </div>

          <NavLink to="/grwmrr" className="sm:text-[16px] md:text-[20px] font-semibold xl:text-[24px]" onClick={handleClose}>GRWMRR</NavLink>
          <NavLink to="/contact" className="sm:text-[16px] md:text-[20px] font-semibold xl:text-[24px]" onClick={handleClose}>CONTACT US</NavLink>
